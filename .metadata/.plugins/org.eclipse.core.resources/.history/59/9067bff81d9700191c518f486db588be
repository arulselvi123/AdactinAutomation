...
String valueOf = String.valueOf(arg1);
dropDownForNum(pm.getLa().getNoOfRooms(), valueOf);
		}*/
		
		@When("^User select the Number of Rooms  '(\\d+)'$")
		public void user_select_the_Number_of_Rooms(int arg1) throws Throwable {
			//String valueOf = String.valueOf(arg1);
			//dropDownForText(pm.getLa().getNoOfRooms(), "valueOf");
		}
		...
		@Then("^User verify the details are not editable$")
		public void user_verify_the_details_are_not_editable() throws Throwable {
 		}
		@Then("^User verify the booked itinerary reflects the correct information$")
		public void user_verify_the_booked_itinerary_reflects_the_correct_information() throws Throwable {
 		}
...
dropDownForNum(pm.getLa().getNoOfRooms(), arg1);	
		...
		public void user_verify_that_all_data_are_same_in_booking_confirmation_page(String arg1, String arg2, String arg3, String arg4, String arg5, int arg6, int arg7, String arg8) throws Throwable {
			...
			Assert.assertEquals(arg1, bconfHotel);
			Assert.assertEquals(arg2, bconfLocation);
			Assert.assertEquals(arg3, bconfRoomType);
			Assert.assertEquals(arg4, bconfCheckin);
			Assert.assertEquals(arg5, bconfCheckout);
			
String stringNoOfRooms = String.valueOf(bconfNoofRooms);	
String stringAdults = String.valueOf(bconfAdults);
			if (stringNoOfRooms.equals(arg6)) {
				System.out.println("No of rooms is same as selected");
			} else {
				System.out.println("No of rooms is not same as selected");
			}
			if (stringAdults.equals(arg7)) {
				System.out.println("No of adults is same as selected");
			} else {
				System.out.println("No of adults is not same as selected");
			}
			if (arg8.contains(bconfChildren)) {
		...
		public void user_confirms_that_order_no_is_generated_or_not_in_Booking_Confirmation_page() throws Throwable {
		    String orderID = getAttributeOfTheElement(pm.getLa().getOrderNoBookConfirm());
		    if(orderID!=null) {
		    	System.out.println("OrderID is  generated"+orderID);
		    }else {
		...
		@Then("^User Logout from Booking Confirmation Page$")
		public void user_Logout_from_Booking_Confirmation_Page() throws Throwable {
		    clickOnTheElement(pm.getLa().getLogoutBookConfirm());
		}
		@Then("^User verify with existing order id '\"([^\"]*)\"' '\"([^\"]*)\"' '\"([^\"]*)\"''\"([^\"]*)\"'$")
		public void user_verify_with_existing_order_id(String arg1, String arg2, String arg3, String arg4) throws Throwable {
		List <WebElement> datas = driver.findElements(
			...
			for (int i = 0; i < 14; i++) {
				for (int j = 1; j <= 15; j++) {
					String values = getAttributeOfTheElement((datas.get(i)));
